...
# Get gifts by draw
@router.get("/draw/{draw_id}", response_model=List[schemas.gift.GiftOut])
def get_gifts_by_draw(draw_id: int, db: Session = Depends(get_db)):
    return db.query(models.gift.Gift).filter(models.gift.Gift.draw_id == draw_id).all()
...
def update_gift(gift_id: int, update: schemas.gift.GiftUpdate, db: Session = Depends(get_db)):
    gift = db.query(models.gift.Gift).filter(models.gift.Gift.gift_id == gift_id).first()
    if not gift:
